...
test('createStore.setUi / update: Anzeigezustand im Store; silent ohne Benachrichtigung; update setzt Filter und UI mit einer Benachrichtigung', () => {
  ...
  let calls = 0;
  store.subscribe(() => { calls += 1; });
  assertEqual(store.getState().ui, { benchmark: 'bank', dq: null, compare: null, snapshots: [], snapshotErrors: [], personen: null, sort: null, editMode: false, fokus: null, vergleichsbanken: [] });
